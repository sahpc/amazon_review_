...
def validate_review(text):
    ...
    special_ratio = len(re.findall(r'[^a-zA-Z0-9áéíóúÁÉÍÓÚñÑ\s]', text)) / max(len(text), 1)
    ...
    suspicious_patterns = [
        "good good", "nice nice", "test test", "muy bueno good good",
        "aaaa", "xxxxx", "fake review", "asdf", "lorem ipsum"
    ]
    text_lower = text.lower()
    for pattern in suspicious_patterns:
        if pattern in text_lower:
            return False, "Posible spam o contenido artificial detectado."
...
def sentiment_label(score):
    ...
    elif score <= -0.6: return "Muy Negativo"
    elif score <= -0.2: return "Negativo"
    return "Neutral"

def review_quality(probability):
    if probability >= 85: return "Excelente"
    elif probability >= 70: return "Buena"
    elif probability >= 50: return "Regular"
    return "Baja"
...
# BUTTON
# =========================================================
if st.button("Analizar Reseña", use_container_width=True):
    ...
    if not is_valid:
        ...
        try:
            ...
            if features.get("coherence", 1) == 0:
                st.warning("La IA detectó posible incoherencia textual.")
            ...
            # AI TRUST SCORE
            ai_score = round(((probability * 0.6) + (word_count * 1.5) + ((sentiment + 1) * 20)) / 2, 2)
            ...
            with col1:
                st.metric("Probabilidad Útil", f"{probability:.2f}%")
            with col2:
                st.metric("Cantidad de Palabras", word_count)
            with col3:
                st.metric("Sentimiento", sentiment_text)
            with col4:
                st.metric("Calidad", quality)
            with col5:
                st.metric("AI Trust Score", f"{ai_score}%")

            # PROGRESS
            st.subheader("Score de Calidad")
            progress_value = max(0, min(int(probability), 100))
            st.progress(progress_value)
            st.caption(f"Nivel de utilidad detectado: {progress_value}%")
            ...
            if sentiment > 0.7 and word_count > 15:
                insights.append("Sentimiento altamente positivo detectado.")
            if word_count > 80:
                insights.append("La reseña contiene información detallada.")
            if features.get('uppercase_ratio', 0) > 0.10:
                insights.append("Posible patrón spam detectado.")
            if probability < 60:
                insights.append("La reseña presenta baja utilidad informativa.")
            if features.get("coherence", 1) == 1:
                insights.append("La IA detectó coherencia textual adecuada.")
            if not insights:
                insights.append("No se detectaron anomalías importantes.")
...
if st.session_state.history:
    ...
    st.dataframe(
        history_df.sort_values(by="probability", ascending=False),
...
try:
    ...
    if response.status_code == 200:
        ...
        with col2:
            st.info(f"Modelos cargados: {health['models_loaded']}")
    else:
